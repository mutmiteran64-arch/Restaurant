...
Le dépôt est configuré pour un déploiement GitHub Actions. Dans Settings > Pages, choisir **GitHub Actions** comme Source. Le nom du dépôt attendu est `Restaurant`, donc Vite utilise `base: '/Restaurant/'`.

## Configuration GitHub Pages obligatoire

Dans GitHub : Settings → Secrets and variables → Actions → New repository secret, ajoute :

- `VITE_SUPABASE_URL` = URL publique de ton projet Supabase
- `VITE_SUPABASE_ANON_KEY` = clé anon publique de ton projet Supabase

Le workflow injecte ces secrets pendant `npm run build`. Sans eux, l’application affiche un message explicite au lieu d’une page blanche.
