    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-secondary-500 mt-1">Les ventes consomment les ingrédients selon les recettes. Les achats entrent en stock uniquement à la réception.</p>
        </div>
        {tab === 'ingredients' && (
          ...
          <table className="w-full">
            <thead className="bg-secondary-50 border-b border-secondary-100">
              <tr>
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-secondary-500 uppercase tracking-wide">Date</th>
                <th className="text-right px-4 py-3 text-xs font-semibold text-secondary-500 uppercase tracking-wide">Action</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-secondary-50">
              {purchases.map((po) => (
                <tr key={po.id} className="hover:bg-secondary-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-right">
                    {po.status !== 'received' && po.status !== 'cancelled' ? (
                      <button
                        onClick={async () => {
                          if (!window.confirm(`Réceptionner la commande d'achat #${po.po_number} ? Le stock sera augmenté et le coût moyen recalculé.`)) return;
                          const { error } = await supabase.rpc('receive_purchase_order', { p_purchase_order_id: po.id });
                          if (error) window.alert(error.message);
                          else loadData();
                        }}
                        className="btn-secondary text-xs"
                      >
                        <CheckCircle2 className="w-4 h-4" /> Réceptionner
                      </button>
                    ) : null}
                  </td>
                </tr>
      ...
      {showInventoryModal && (
        <InventoryModal
          ...
          onSaved={() => { setShowInventoryModal(false); loadData(); }}
        />
      )}
      {showMovementModal && (
        <MovementModal
...
function InventoryModal({
  ...
  const [counts, setCounts] = useState<Record<string, string>>(() =>
    Object.fromEntries(ingredients.map((i) => [i.id, String(i.quantity)]))
  );
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  async function handleSave() {
    ...
    try {
      for (const ing of ingredients) {
        const value = Number(counts[ing.id]);
        if (!Number.isFinite(value) || value < 0) throw new Error(`Quantité invalide pour ${ing.name}`);
        const { error: rpcError } = await supabase.rpc('adjust_physical_inventory', {
          p_ingredient_id: ing.id,
          p_counted_quantity: value,
          p_reference: 'Inventaire physique',
        });
        if (rpcError) throw rpcError;
      }
      onSaved();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Erreur pendant l’inventaire');
      setSaving(false);
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-secondary-950/40 p-4">
      <div className="card p-6 max-w-2xl w-full max-h-[85vh] overflow-y-auto animate-slide-up">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-xl font-bold text-secondary-900">Inventaire physique</h2>
            <p className="text-sm text-secondary-500">Saisissez les quantités réellement comptées. Seul l’écart sera enregistré.</p>
          </div>
          <button onClick={onClose} className="text-secondary-400 hover:text-secondary-600"><X className="w-5 h-5" /></button>
        </div>
        <div className="space-y-2">
          {ingredients.map((ing) => (
            <div key={ing.id} className="grid grid-cols-12 gap-3 items-center p-3 rounded-lg bg-secondary-50">
              <div className="col-span-6">
                <p className="text-sm font-medium text-secondary-900">{ing.name}</p>
                <p className="text-xs text-secondary-500">Stock théorique : {formatNumber(ing.quantity, 3)} {ing.unit}</p>
              </div>
              <input
                type="number"
                step="0.001"
              ...
              />
              <span className="text-xs text-secondary-500 col-span-2">{ing.unit}</span>
            </div>
          ))}
        </div>
        {error && <div className="mt-4 text-sm text-error-600 bg-error-50 border border-error-200 rounded-lg px-3 py-2">{error}</div>}
        <button onClick={handleSave} disabled={saving || ingredients.length === 0} className="btn-primary w-full mt-5">
...
function MovementModal({
  ...
  async function handleSave() {
    ...
    if (form.quantity <= 0) { setSaving(false); return; }

    if (form.type === 'waste' || form.cause === 'waste' || form.cause === 'breakage') {
      const { error } = await supabase.rpc('record_stock_loss', {
        p_ingredient_id: ing.id,
        p_quantity: Math.abs(form.quantity),
        p_cause: form.cause === 'breakage' ? 'breakage' : 'waste',
      ...
      const newQty = ing.quantity + signedQty;
      if (newQty < 0) { window.alert('Stock insuffisant'); setSaving(false); return; }
      const { error: updateError } = await supabase.from('ingredients').update({ quantity: newQty }).eq('id', ing.id);
      if (updateError) { window.alert(updateError.message); setSaving(false); return; }
      ...
      if (movementError) { window.alert(movementError.message); setSaving(false); return; }
    }
    setSaving(false);
    onSaved();
  }
...
function PurchaseModal({
  ...
  async function handleSave() {
    ...
    if (poData && items.length > 0) {
      ...
      // Important: a purchase order does NOT enter stock until it is physically received.
      // Reception is performed atomically by receive_purchase_order().
    }
