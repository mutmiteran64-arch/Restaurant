...
export default function Reports() {
  ...
  const [topProducts, setTopProducts] = useState<{ name: string; qty: number; rev: number; cost: number }[]>([]);
  ...
  async function loadReport() {
    ...
    const [orders, orderItems, payments, financial] = await Promise.all([
      supabase.from('orders').select('id, total, server_name, table_name, created_at, status').eq('status', 'paid').gte('created_at', startDate),
      supabase.from('order_items').select('product_name, quantity, total_price, cost_total').gte('created_at', startDate).eq('status', 'served'),
      supabase.from('payments').select('method, amount').gte('created_at', startDate),
      supabase.rpc('get_financial_summary', { p_start: startDate, p_end: new Date().toISOString() }),
    ]);
    ...
    setCogs(Number(financial.data?.cogs ?? 0));
    setGrossProfit(Number(financial.data?.gross_profit ?? 0));
    setFoodCostPercent(Number(financial.data?.food_cost_percent ?? 0));

    // Top products
    const prodMap = new Map<string, { qty: number; rev: number; cost: number }>();
    (orderItems.data ?? []).forEach((item) => {
      const ex = prodMap.get(item.product_name) ?? { qty: 0, rev: 0, cost: 0 };
      ex.qty += item.quantity;
      ex.rev += Number(item.total_price);
      ex.cost += Number(item.cost_total ?? 0);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="card p-5">
          <div className="flex items-center gap-3 mb-2">
            ...
            <p className="text-sm text-secondary-500">Food cost</p>
          </div>
          <p className="text-2xl font-bold text-secondary-900">{formatCurrency(cogs)}</p>
          <p className="text-xs text-secondary-500 mt-1">{foodCostPercent.toFixed(1)}% du CA</p>
        </div>
        <div className="card p-5">
          <div className="flex items-center gap-3 mb-2">
            <div className="w-10 h-10 rounded-lg bg-accent-50 flex items-center justify-center">
            ...
            <p className="text-sm text-secondary-500">Marge brute</p>
          </div>
          <p className="text-2xl font-bold text-secondary-900">{formatCurrency(grossProfit)}</p>
        </div>
      </div>
