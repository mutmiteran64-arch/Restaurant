...
function ProductModal({
  ...
  async function handleSave() {
    ...
    const productData = {
      ...
      price: form.price,
      cost: recipes.length > 0 ? computedCost : form.cost,
  ...
  function addRecipeIngredient() {
    if (!selectedIngredient || recipeQty <= 0) return;
    const ing = ingredients.find((i) => i.id === selectedIngredient);
    if (!ing) return;
    if (recipes.some((r) => r.ingredient_id === ing.id)) {
      setSaveError(`${ing.name} est déjà présent dans la recette.`);
      return;
    }
      ...
      <div className="card p-6 max-w-lg w-full animate-slide-up my-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-xs text-secondary-400 mt-1">Échap ou × pour fermer sans enregistrer</p>
          </div>
          <button
            type="button"
            onClick={onClose}
            ...
            className="w-9 h-9 rounded-lg flex items-center justify-center text-secondary-500 hover:bg-secondary-100 hover:text-secondary-900 disabled:opacity-50"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="border-t border-secondary-100 pt-4">
            ...
            <div className="rounded-lg bg-primary-50 border border-primary-100 px-3 py-2 mb-3 text-xs text-primary-700">
              <strong>Comment le stock est lié :</strong> chaque ligne ci-dessous représente une matière première du stock.
              À chaque vente de ce produit, la quantité indiquée dans la recette sera automatiquement déduite du stock.
            </div>
            <div className="flex gap-2 mb-3">
              ...
              >
                <option value="">Choisir un ingredient du stock...</option>
                {ingredients.map((ing) => (
