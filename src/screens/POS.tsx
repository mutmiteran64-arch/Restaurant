...
export default function POS() {
  ...
  async function loadData() {
    ...
    setTables(tbls.data ?? []);
    if (cats.data && cats.data.length > 0) setSelectedCategory(cats.data[0].id);
  }
  ...
  async function handleCheckout(method: 'cash' | 'card' | 'mobile_money') {
    ...
    try {
        ...
        if (recipes) {
          for (const recipe of recipes) {
            ...
            const { data: ing } = await supabase
              .from('ingredients')
              .select('quantity, name, cost_per_unit')
              .eq('id', recipe.ingredient_id)
              .maybeSingle();
            const newQty = Math.max(0, (ing?.quantity ?? 0) - deductQty);
            await supabase.from('ingredients').update({ quantity: newQty }).eq('id', recipe.ingredient_id);
      ...
      if (selectedTable) {
        await supabase.from('restaurant_tables').update({ status: 'cleaning' }).eq('id', selectedTable);
      }
      ...
      await supabase.from('audit_logs').insert({
        ...
        entity_type: 'order',
        entity_id: order.id,
        details: { order_number: orderNumber, total, method, items: cart.length },
      });

      setShowReceipt(order as Order);
      setCart([]);
      ...
      setError(err instanceof Error ? err.message : 'Erreur lors du traitement de la commande');
    } finally {
      setSubmitting(false);
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
            {filteredProducts.map((product) => (
              <button
                ...
                className="card p-4 text-left hover:shadow-md hover:border-primary-200 transition-all active:scale-[0.98] group"
              >
